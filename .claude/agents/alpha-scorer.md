...
절차:
1. 3쌍 pairwise α 산출: `alpha.py rel`을 (a,b)/(a,c)/(b,c) 세 번 실행 — **G1 게이트 = 차원별 최솟값** (codebook v1.2 §9; D1/D2/D4 nominal, D3 ordinal). 추가로 3-way joint `rel a b c` 병기. 특정 쌍의 비정상 고일치는 상관오류 적신호로 리포트에 명기.
