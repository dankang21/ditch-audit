...
| Coder D (archival, **mandatory** — **CONFIRMED by dk 2026-07-17**) | **`qwen3.5:4b` (Alibaba/Qwen lineage), locally served via Ollama; digest `2a654d98e6fb` pinned; Q4_K_M quantization of record; weights held on local disk (archival requirement met in its strongest form)**. Frozen runtime: `think:false`, temperature 0, num_ctx 16384. Protocol mirrors A/B/C (majority-of-3). Smoke-validated 2026-07-17: schema-valid single-shot JSON, 14 s/item CPU, dummy-item codes identical to the frontier trio | re-executability after endpoint retirement; results reported, not gating |
