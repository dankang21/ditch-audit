...
Our contribution is accordingly of an unusual type for the philosophy of religion: measurement of the dialectic, not participation in it. The paper makes one framework claim and two empirical claims. The framework claim (\S{}2) is that a witness-based criterion individuates the chain into a canonical eight-step partition, and (\S{}7) that the count of independent evidential burdens this partition exposes is invariant under logical and definitional repackaging --- bundling the steps into ``a maximally great being exists'' relocates the burdens into the bundle's prior; it does not reduce them. The empirical claims are two preregistered hypotheses about the shape of the published record, described below; their preregistered test is reported in \S{}5, where the gate chain halted at its first item before either was run.
...
The paper proceeds as follows. Section 2 states the individuation criterion. Section 3 applies it, yielding the canonical eight-step partition (Table 1), and explains in what sense the table's qualitative status verdicts are priors the audit was designed to test --- and, if need be, overturn. Section 4 describes the corpus, the codebook, and the zero-human coding pipeline; \S{}5 reports results; \S{}6 the robustness battery; \S{}7 the formal treatment of burden invariance, positioned against Roche and Shogenji's (2014) result on confirmation transmission; \S{}8 answers five objections; \S{}9 states what the map licenses.
...
\item
  \textbf{Non-derivability, witness-certified.} S\_i is not derivable from S\_1, \ldots, S\_\{i$-$1\}; and this independence is certified by a \textbf{witness} --- an extant scholarly position that accepts all prior steps and rejects S\_i.
\item
...
\textbf{Table 1. The canonical eight-step partition, with design-phase priors to be tested (see text).}
...
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Witness (accepts 1..i$-$1, rejects i)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
S3 & The agent is unique, a se, uncreated; all else ontologically depends on it (\emph{pantokrator}) & Bostrom's simulator (2003); Hartshorne 1984; Mormon finitism & gap-problem literature: Pruss 2009; Rasmussen 2009; Ocampo 2024 $\cdot$ bundling fence: Geach 1973; Hasker 1989 & contested / frozen --- evidence up to S2 shared with contingent-creator hypotheses & $\alpha$ \\
S4 & The ultimate is perfectly good & Draper's indifference; Law's evil-god; Mill's finite god (1874) & contra: Rowe 1979; Draper 1989; Law 2010; Cahn 1977 $\cdot$ pro anchors: defensive items only (Plantinga 1974; Wykstra 1984) & headwind & $\alpha$+$\beta$ (Draper's likelihood argument has data contact) \\
S5 & The good ultimate acts specially in the world post-creation (miracles, answered prayer) & Wiles; deism & contra: Cochrane review CD000368 (2009); STEP trial (2006); Wildman 2004; Hume $\cdot$ pro anchors: coherence defenses only & strong headwind; the chain's only fully live empirical channel & $\beta$ \\
S6 & The authentic bearer of that God's special revelation is the Hebrew prophetic tradition & Islam; Judaism; Hick's pluralism (1989) & contra: Alston's parity argument (1991) $\cdot$ pro anchor: Plantinga 2000 (a permissibility claim) & headwind / near-frozen; evidence defers to S7 & $\beta$$\cdot$$\gamma$ \\
S7 & Jesus of Nazareth was bodily raised (\textasciitilde AD 30) & Lüdemann (1994); Allison's \emph{non liquet} (2021) & pro: Wright 2003; Swinburne 2003 (with its own dwindling-probabilities caveat) & strong headwind; evidence-frozen (fixed corpus) & $\gamma$ \\
S8 & The resurrection shows Jesus is God incarnate; God is triune & Lapide (accepts S1--S7, rejects S8); Tuggy 2003 & pro-coherence: Morris 1986; Brower \& Rea 2005 & defensive; truth-evidence fully S7-dependent & $\alpha$ ($\gamma$-dependent) \\
\end{longtable}

The qualitative-prior column requires immediate discipline. Its verdicts --- \emph{contested}, \emph{headwind}, \emph{frozen}, \emph{defensive} --- originate in the qualitative reading of the anchor literatures that produced this design. They are not results, and the audit does not assume them. They are priors, and they are \emph{computable}: a frozen, ordered decision list (applied in \S{}5.6) defines each verdict as a function of coded aggregates within the mainstream tiers --- a step is \textbf{contested} when both directions field at least \emph{k} positive evidential (POS) items (default \emph{k} = 3, reported at \emph{k} $\in$ \{2, 3, 5\}); a \textbf{headwind} obtains when POS items are one-sided, the opposing direction fielding none, whatever its defensive (DEF) activity --- the predicate is direction-unsigned, and the computed status reports which direction fields the POS items; a step is \textbf{frozen} when neither side has produced a POS item in the trailing ten years; residual cells report as mixed/sparse. The audit therefore either reproduces this column or corrects it, cell by cell, and either outcome is reported.
...
Coverage was closed by a sequence of recovery passes, all verbatim: the recovery scripts copy source text with whitespace collapsing only, and no generative or search-augmented model touches the abstract-recovery path. First, database recovery pulled verbatim abstracts from OpenAlex and Semantic Scholar (293 \emph{Faith and Philosophy} abstracts came from OpenAlex alone), and a gap in Crossref's \emph{Faith and Philosophy} deposit for 2022--2023 was backfilled from OpenAlex with issue years derived from volume numbers (year = 1983 + volume; volume 41, i.e., 2024, is absent from the source itself --- unpublished, not unharvested), adding 65 items and lifting overall coverage to 73.2\%.
...
\textbf{Deduplication and identifier integrity.} All merging is DOI-keyed, and every pipeline stage logs an identifier-uniqueness check: the final T2$\cup$T3 corpus contains 3,362 unique DOIs in 3,362 rows with zero cross-journal duplicates; the T1 candidate set 296 unique in 296; the APQ backfill 151 unique identifiers in 151 rows (8 DOIs plus OpenAlex work IDs).
...
Disagreements are resolved by mechanical majority-of-three; three-way splits become \texttt{unresolved}, excluded from primary analysis and bounded by recomputation under every candidate resolution. Because majority voting can launder correlated error, the battery instruments the majority itself. A conditional-agreement diagnostic, P(B = C \textbar{} A $\neq$ B), is computed per dimension at the confirmatory checkpoint and midpoint (pilot baseline .45--.75 --- no lockstep bloc), with a frozen alarm ($\ge$ 0.90 with conditioning n $\ge$ 20, sustained over two consecutive evaluations) and a frozen response: coding pauses, the adjudicator seat switches to the registered alternative family, all adjudications are mechanically recomputed, the event is logged as a preregistered deviation. No open-ended re-examination is available, because ``the designers will look into it'' is not a preregistrable consequence.
...
The pilot reliability gate passed on its preregistered statistic --- minimum pairwise Krippendorff's $\alpha$ of .736/.727/.808/.705 across the four dimensions at n = 89 --- but the gate report itself documents why that estimate cannot be taken at face value: it is a \emph{selected} estimate, reached on the fifth round of a revision loop with the gate in view (an optional-stopping structure); its interval evidence sits near rather than safely above the floor (8 of 12 pair$\times$dimension bootstrap cells with a 95\% lower bound below .70); and a gold-exclusion sensitivity dropped three cells marginally under .70, confirming that recognition-advantaged anchors contributed to the pass. The gate declaration was therefore made \emph{conditional}, by its own banner, on a confirmatory checkpoint at the main coding phase: 150 fresh items with zero pilot overlap, drawn from held-out venue-years by a frozen, seeded, stratified venue$\times$year random draw (script and seed are manifest artifacts --- ``the first 150 items'' is undefined without them), coded in both the raw and laundered arms. The checkpoint statistic is dual, both parts required on every dimension: point minimum-pairwise $\alpha$ $\ge$ .70, \emph{and} the per-dimension bootstrap 95\% lower bound of the minimum-pairwise $\alpha$ --- four interval quantities, the minimum over pairs recomputed inside every replicate --- $\ge$ .667, Krippendorff's floor for tentative conclusions. The sample size makes the checkpoint load-bearing rather than decorative: at n = 150 a true-$\alpha$ $\approx$ .78--.80 instrument passes the interval criterion comfortably while a true-$\alpha$ $\approx$ .70 instrument fails it. Failure halts coding entirely, with no confirmatory claims from any already-coded material.

The registered reporting frame is candid about what this buys: \emph{tentative-band reliability, checkpoint-confirmed}. Because $\alpha$ $\approx$ .70 admits substantial misclassification, and only nondifferential misclassification reliably attenuates odds ratios, a quantitative misclassification sensitivity is preregistered --- the primary effect recomputed under nondifferential and direction-differential error scenarios spanning the checkpoint's observed disagreement matrices, with a frozen ``reliability-sensitive'' qualifier attached to any verdict that does not survive the plausible-differential range. The same candor governs the criterion set: the plan names the gap between near-ceiling development-half accuracy (97--100\%) and real-corpus pairwise reliability (.70--.74) as evidence that synthetic items are easier than real ones --- ``near-ceiling construct validity'' is weaker than it sounds, the natural experiment bounds contamination but not difficulty mismatch, and the gap is carried as a quantified limitation.

\subsubsection{4.4.8 Power and yield}\label{power-and-yield}

Power planning rests on a pre-freeze calibration sample rather than the design outline's early sketch, which pilot rates showed optimistic by roughly a factor of five. A seeded, stratified sample of 150 abstract-bearing specialist-corpus items, coded single-run by one coder family for calibration only (its confirmatory disposition is itself a registered decision), yields P(chain-step-coded) = 39.3\% {[}31.9, 47.3{]} and P(positive-evidential \textbar{} chain-step) = 16.9\% {[}9.5, 28.5{]}: an effective rate of 6.67\%, hence roughly 197 expected positive-evidential items {[}conservative 89, optimistic 398{]} on the 2,958 covered abstracts, before general-venue additions and before the full-text second pass, which can only add. The plan corrects the figure per denominator (the type--direction analysis admits mainstream venues only, giving $\approx$ 162--190 expected items) and freezes a two-stage yield trigger: projected yield below 110 --- where power for the registered effect size crosses $\approx$ .77 --- forces harvest expansion before further spend, following a deterministic registered preference order with no discretionary choice. The mid-run yield check is performed by a firewalled script whose only surfaced outputs are the trigger boolean and marginal counts, never cross-tabulations of the quantities under test: an interim look that can change the population definition is defensible only if provably blind to the association under test and deterministic in response. The venue-topography hypothesis, unpowered in the first draft, receives registered expected counts for all eight critical cells and a registered power computation at freeze, with a frozen contingency re-scoping its confirmatory element to the main-effect contrast if interaction power falls below .5 --- decided before any confirmatory data exist, the kill criterion re-read against the re-scoped element.
...
halted (\S{}5.1) before the component's confirmatory arm ran; ``moot under gate FAIL'' marks the
two components interrupted when coding stopped (\S{}5.1.1 discloses the proximate interrupter).
...
preregistered extension carrying their own gate (\S{}5.5). Second, all confirmatory verdicts are
...
\textbf{P4 resolution (added 2026-07-17, dated).} The gate chain below resolved at its first item:
the confirmatory reliability checkpoint failed on all four dimensions (\S{}5.1.1). Under the
frozen decision tree, no confirmatory coding was performed, no outcome data exist, and every
downstream shell in this section resolves to \emph{not run} rather than to a filled cell. Per the
...
component results, including failures, are published (battery \S{}B10).
...
Gate: point min-pairwise $\alpha$ $\ge$ .70 on every dimension \textbf{and} per-dimension bootstrap 95\% lower
bound $\ge$ .667. Statistic 1 failed on D2 and D3; statistic 2 failed on all four dimensions.
...
family-correlation watch (plan \S{}6.3) read P(B = C \textbar{} A $\neq$ B) between .41 and .52 on all dimensions,
...
revisions under an optional-stopping structure, and \S{}6 of the frozen plan states that ``a
true-$\alpha$ $\approx$ .70 instrument will fail statistic 2 --- which is the intended behavior (the checkpoint
...
OR\_H1 with Woolf 95\% CI: \emph{not run}. Fisher exact p: \emph{not run}.
Registered prediction: OR\_H1 $\ge$ 3.
...
OR\_pro: \emph{not run} $\cdot$ OR\_contra: \emph{not run} $\cdot$ ROR with 95\% CI:
\emph{not run}. Registered predictions: (i) OR\_pro \textgreater{} 1; (ii) OR\_contra $\approx$ 1 or \textless{} 1;
(iii) ROR \textgreater{} 1.
...
n $\ge$ 15, and no confirmatory ROR may rest on a Haldane-corrected analysis-critical cell. If the
registered pre-freeze ROR power computation fell below .5, H2's confirmatory element is the
re-scoped OR\_pro test, and this subsection reports against the re-scoped element as registered.
Verdict returned by the frozen rule: \emph{not run}.
...
{[}0.5, 2{]} --- applies to the directional effects only (OR\_H1, OR\_pro, ROR); OR\_contra is exempt
because it is predicted $\approx$ 1, and is published descriptively.
...
Alternative-seat co-analysis (adjudicator = A): \emph{not run}. Frozen divergence rule: if
...
Era$\times$venue bias table --- rows = venue, columns = era (2004--2007, 2008--2024); cells = item count,
abstract coverage, and abstract-free covariate profiles (article type, frozen-keyword topic
profile, page count): \emph{not run}.
...
checkpoint-tranche adjudicated distributions): \emph{not run}. A whole-window extension
...
Computed step status per step over T1$\cup$T2 within window, by the frozen ordered decision list ---
(1) contested: both directions $\ge$ k POS items; (2) headwind: exactly one direction $\ge$ k POS and
...
(4) mixed/sparse: residual --- reported at k $\in$ \{2, 3, 5\}:
...
designers twice during construction (\S{}4.3.5). In the A11 episode, the act of gold-coding caught a
...
Also registered for this subsection, all \emph{not run}: per-step direction $\times$ strength $\times$ type maps;
venue-tier topography maps; B-prevalence (input to \S{}7); the S6 scarcity check; flag-class
...
no post-hoc hypothesis swaps. The descriptive map (\S{}5.6) and the full battery results are
...
\textbf{Realized branch (added at P4 resolution, 2026-07-17).} None of the four scenarios activated.
...
\item
  ...
  \emph{not run}
\item
  R2 --- \emph{Faith and Philosophy} tier flip. H2 recomputed with \emph{Faith and Philosophy} assigned T3; both assignments
  published; the frozen tier table is primary. \emph{not run}
\item
  R3 --- dominant-author exclusion. Items by Swinburne, Plantinga, Draper, Oppy dropped
  (metadata join, post-coding); H1/H2 recomputed. \emph{not run}
\item
...
\item
  ...
  robustness-only by preregistered fiat. \emph{not run}
\item
  B7a --- reversed-default deletion test. D4 re-derived under reversed tie-break polarity;
  tie-break-decided fraction reported; H1 under both polarities. \emph{not run}
\item
...
\begin{quote}
P(T \textbar{} E) = $\prod_i$ P(S\_$\sigma$(i) \textbar{} S\_$\sigma$(1), \ldots, S\_$\sigma$(i$-$1), E). (1)
\end{quote}
...
\begin{quote}
P(T \textbar{} E) = $\prod_j$ P(B$_j$ \textbar{} B$_1$, \ldots, B\_\{j$-$1\}, E), (2)
\end{quote}
...
\textbf{Proposition 1 (burden invariance).} Fix a partition (S$_1$, \ldots, S$_n$) of T. Under every ordering and every coarsening into blocks, the evaluation of P(T \textbar{} E) resolves into exactly n cell-level conditional commitments, one per cell. The choice of representation determines only \emph{where} each commitment appears --- as an explicit factor in the displayed product, or inside a bundle's conditional --- never \emph{how many} there are.
...
\textbf{Corollary 1 (anti-bundling).} Definitional bundling cannot reduce the number of evidential burdens. If a bundled predicate G --- ``maximally great being'' is the canonical example --- is logically equivalent to a conjunction of cells, then P(G \textbar{} $\cdot$, E) simply \emph{is} the product of the corresponding cell conditionals, now paid inside a single symbol; if G is logically stronger than the conjunction, its probability is bounded above by the conjunction's, and the bundling has made the position formally worse, not better. Either way the burdens are relocated, not retired.
...
The multiplicative structure of (1) is not news to the literature; it is the engine of the dwindling-probabilities exchange. Plantinga (\emph{Warranted Christian Belief}, 2000) argued that the probability of full Christian belief, propagated through a chain of stages on historical evidence, dwindles multiplicatively to unimpressive values. Swinburne's reply (\emph{Faith and Philosophy}, 2004b) charged the argument with mishandling how evidence enters at each stage; T. McGrew (\emph{Philosophia Christi}, 2004) pressed a structurally similar objection; Plantinga replied in 2006 (\emph{Philosophia Christi} 8(1), 7--22) --- with, as the secondary literature reports, a partial concession {[}the concession characterization remains secondary-sourced; primary-text check stays on the must-verify register{]} --- followed by McGrew and McGrew (\emph{Philosophia Christi} 8(1), 2006) and, later, Nickel (De Gruyter, 2015).
...
Three admissions, restated compactly. (i) \emph{Elementarity.} Proposition 1 is the chain rule read with an accountant's eye; the paper stakes no claim to mathematical novelty, and its venue case does not rest on formal depth. (ii) \emph{Axiological silence.} Nothing here shows any burden to be heavy; the non-claims of \S{}1 apply with full force, and the appendix is compatible with every burden being paid. The paper takes no position on the truth of any S$_i$. (iii) \emph{Grain-relativity.} The integer ``eight'' is relative to the witness criterion and to the state of the record that certifies it; the grain objection (\S{}8, O1) lives exactly here. The reply is twofold: the paper's empirical hypotheses are formulated as invariants across admissible grains and are subjected to a preregistered grain-robustness check (R1, including the Mill finite-god cut); and the canonicity argument above ties admissible grains to occupied positions, so that the residual freedom in ``eight'' is the record's, not the authors'. What the formalism licenses, in the end, is modest and precise: a cumulative case may be stated in any vocabulary its proponent likes, but its burden ledger is representation-invariant, and the ledger's line items are fixed by the dialectical facts on the ground --- facts the remainder of this paper undertakes to measure rather than presume.
...
says so. The individuation criterion (\S{}2) admits a step only where a witness exists --- an extant
...
analyst's taste. The formal section (\S{}7) shows that the burden multiset is invariant under
...
(\S{}6) codes a three-step naturalism mini-chain with the same instrument architecture, the same
...
\S{}4.3.5 and \S{}5.6), and the design treats designer error as a measurable, correctable event rather than an
...
\textbf{Scope note at P4 resolution (2026-07-17).} No map was produced: the gate chain halted at
...
licenses is confined to the methodological findings of \S{}5.1.1, the design contributions of
\S{}\S{}2--4, and the formal result of \S{}7 (burden invariance), which stands independently of the
failed coding run.
...
gatekeeping discussion (\S{}8, O3) concerns what could explain a published pattern, not what
...
rule --- under which the coders twice overrode the designers; see \S{}4.3.5.) The
...
(\S{}5.1.1), and the halt executed as designed. Second, the full design
...
Every preserved shell (\S{}\S{}5.2--5.7, \S{}6) is byte-intact from v0.2; body changes are confined to
...
(5) \S{}4.4.10 added --- a battery-at-a-glance table that rearranges numbers already stated in
\S{}\S{}4.4.1--4.4.7 and \S{}5.1.1, with no new measurements; (6) two bibliographic corrections found by
web verification and applied to the body: \S{}7.3's ``Plantinga replied in 2004'' corrected to 2006
(\emph{Philosophia Christi} 8(1), 7--22, per the publisher record; the partial-concession
...
distinct Swinburne 2004 works disambiguated as 2004a (\emph{The Existence of God}, 2nd ed.~--- the
Table 1 S2 anchor) and 2004b (the \emph{Faith and Philosophy} dwindling-probabilities article ---
\S{}\S{}1, 7.3), and one finding re-attributed to its verified source: the argument-strength-ratings
result moved from the De Cruz (2017) key to De Cruz and De Smedt (2016), with the 2017 study's
actual finding (disagreement assessments track prior belief) stated in its place (\S{}1);
...
At P4 the confirmatory checkpoint failed (\S{}5.1.1) and the PI elected the preregistered
...
reconciliation (commit \texttt{83d0b49}, review-exclusion count 620 $\to$ 627 in \S{}4.1 --- an outcome-blind
...
\item
  ...
  scoped to B1a/B1b;
\item
  \S{}1: two sentences recast (``their preregistered test is reported in \S{}5, where the gate chain
  halted\ldots{}''; ``Two patterns were predicted for test'');
\item
  \S{}4.3.3 and the AI-use disclosure: the zero-human claim scoped to corpus and pipeline items,
...
\item
  \S{}5.1.1 added: checkpoint numbers, sub-audits, the spend-stop disclosure, and the
  selection (preregistered) vs domain-shift (post-hoc) readings;
\item
  all downstream \texttt{{[}TO\ BE\ FILLED\ AT\ P5{]}} markers resolved to \emph{not run} (one \S{}5.6 frame word
...
repaired in place; the section source files (\texttt{docs/draft/0*.md}) remain unmodified pre-audit
snapshots. Repairs of record: the pre-2008 covered/missing inversion corrected to 462 covered /
210 missing in \S{}4.4.6 and \S{}5.5, matching the same-session correction in analysis-plan v1.1
(\S{}4.1 already stated it correctly); coder C per-run determinism corrected to 80--85\% per
dimension (\S{}4.3.2); the estimator-stability list restored to the battery's five estimators
(\S{}4.3.3); the \emph{IJPR}/\emph{Sophia} residual characterization aligned to the harvest log's prior-pass,
not-individually-source-confirmed verdicts (\S{}4.1); the coder-pin sentence aligned to the
manifest's dated-snapshot requirement, under which the A/B strings are lineage identifiers
(\S{}4.3.2); the prior-art cited-by walks restated as a registered pre-freeze closure step (\S{}1);
...
symmetric over-reading fence added to \S{}9; plus register and date-coherence repairs (\S{}4 intro,
\S{}4.1 review-exclusion scope, \S{}4.2.3 D3 notation, header note). No shell, threshold,
