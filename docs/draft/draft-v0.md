...
Each item is coded independently by three coders from three different model families — A: Anthropic (`claude-opus-4-8` lineage), B: Google (`gemini-3.5-flash` lineage), C: OpenAI (`gpt-5.5-2026-04-23`) — each pinned at freeze in the sealed manifest's pin section: C to an immutable dated snapshot; A and B to provider-stable undated identifiers, for which the sealed manifest records a fallback protocol — per-call version-metadata logging plus the B1c consolidated-determinism floor — in place of the dated-snapshot requirement those providers' identifiers cannot satisfy. The fallback's content differs by provider and is stated exactly: B's responses carry a `modelVersion` field, logged per call; A's calls pin the API-contract version header and log the response's model-identifier echo — the provider exposes no per-response build identifier, a disclosed limit of the fallback that B1c was precommitted to backstop. No coder sees another's output. A fourth, open-weights archival coder (D) is mandatory for re-executability after commercial API endpoints retire; its role, like every other model role in the study, is fixed by a precommitted role-exclusivity matrix. For the record of what D actually did: D coded the 150-item checkpoint tranche, three runs per item — the raw three-run column ships with the audit package, unconsolidated, consolidation having been mooted by the halt; it sits outside the tri-family gate statistic by precommitted role; and no D column exists for the never-run main corpus.
...
Fixation is enforced mechanically: model pins recorded in the sealed manifest (one dated immutable snapshot, C; provider-stable undated identifiers under the precommitted version-metadata-plus-B1c fallback (its per-provider content and limits are stated in §4.3.2), A and B), provider deprecation status checked at freeze with no conflicting retirement announcement identified for the coding-plus-reproduction window, and a precommitted mid-run deprecation protocol (halt, substitute, full recode of the affected coder's column — never mixed within one column). Because the pinned endpoints are not sampling-deterministic, each coder's output is the per-dimension majority of three independent identical runs, and determinism is audited at three levels rather than asserted. **B1a**, per-run exact-match determinism, is reported descriptively with a 70% sanity floor (pilot: A 95–100%, B 85–95%, C 80–85% per dimension). **B1b**, the consolidated run-split rate, is capped at 2% with an alarm at 5% (pilot: one three-way split in 89 items for coder C, 1.1% on each affected dimension; zero for A and B). **B1c**, the binding floor, requires the full majority-of-three pipeline, re-run twice on a 50-item checkpoint subsample, to reproduce the consolidated output at ≥ 95% exact-match per dimension per coder. B1c was not measured at pilot; the plan records the projection from measured 2–1 run-split rates that coder C sits at roughly 94–95% on one dimension, and therefore states *in advance* that the corresponding revision trigger — raising that coder to majority-of-five with mandatory re-consolidation of all its prior items — may fire at P4. A revision trigger whose firing is predicted in the sealed plan cannot later be presented as a surprise or negotiated away.
...
The set is split: a development half (30 items) usable during pilot revision loops, and a vaulted half (30) generated and SHA256-hashed *before any pilot coder run*, opened exactly once at the main coding phase, its hash a freeze artifact. Whatever overfitting five rounds of instrument revision may have induced, it cannot have been fit to items no model call had yet seen. Vaulted-half accuracy floors are frozen at the protocol freeze (85% on step, direction, and strength; 75% on epistemic type — the strength floor raised from a provisional 70% calibrated to a since-collapsed five-level scale, vacuous at two levels where chance is 50%); vault failure at P4 blocks all confirmatory claims. The development half scored 97–100% for all three coders at pilot; §4.4.7 returns to the honest reading of that number.
...
Disagreements are resolved by mechanical majority-of-three; three-way splits become `unresolved`, excluded from primary analysis and bounded by recomputation under every candidate resolution. Because majority voting can launder correlated error, the battery instruments the majority itself. A conditional-agreement diagnostic, P(B = C | A ≠ B), is computed per dimension at the confirmatory checkpoint and midpoint (pilot baseline .45–.75 — no lockstep bloc), with a frozen alarm (≥ 0.90 with conditioning n ≥ 20, sustained over two consecutive evaluations) and a frozen response: coding pauses, the adjudicator seat switches to the precommitted alternative family, all adjudications are mechanically recomputed, the event is logged as a precommitted deviation. No open-ended re-examination is available, because "the designers will look into it" is not a precommittable consequence.

Every headline effect is computed under five estimators — unanimous-only (E0), each single coder (E1–E3), and the primary majority-of-three (E4) — and confirmatory claims require sign stability and confidence-interval overlap across all five, on the directional headline effects only. The contrast effect whose precommitted prediction is approximately 1 is exempt, for a stated reason: an estimate predicted to sit near 1 will chatter across the null under resampling precisely when the design succeeds, so a stability gate keyed to it would fire on success; it is scored solely by its own precommitted decision row, with an evaluability floor.
...
instrument-relative: the sealed plan's frozen sentence form is "H1 [H2] supported, as measured by the
...
parameters and refuses off-spec input, and its hash is recorded in the guardian log.
...
recorded in the sealed plan. What remains is most directly consistent with substantive disagreement among three model
...
*Shell-adjacent resolution note (added v0.8; the shell below is unchanged from the sealed protocol except its dated v0.2 resolution markers, inventoried in the v0.2 note): the shell's phrase "all admissible grains" is the sealed plan's shorthand for R1's three enumerated variants (§7.5(v)); none of this section's items was run (§5.1.1).*
...
admissibility at its precommitted working grain; and the grain-robustness check (R1) exists
precisely because rival admissible partitions may exist.
(v) *Grain-relativity.* The integer "eight" is relative to the witness criterion and to the state of the record that certifies it; the grain objection (§8, O1) lives exactly here. The reply is twofold: the paper's empirical hypotheses were required to survive the precommitted grain variants (R1, including the Mill finite-god cut) — the sealed operationalization, not a quantifier over every admissible grain; the §6 shell's "all admissible grains" is the sealed plan's shorthand for those enumerated variants, and R1, like all of §6, never ran; and the canonicity argument above ties admissible grains to occupied positions, so that the residual freedom in "eight" is the record's, not the authors'. What the formalism licenses, in the end, is modest and precise: a cumulative case may be stated in any vocabulary its proponent likes, but its burden ledger is representation-invariant, and the ledger's line items are anchored in the dialectical facts on the ground, in the certification grades those facts currently support — facts the precommitted design undertook to measure rather than presume; §5 records where the attempt stopped.
...
O4 — the asymmetry objection: "run it on naturalism." The precommitted design answers it within
...
What the precommitted design aimed at is exactly what the subtitle claims: a measurement of the
...
matrix and pinned in the sealed manifest's pin section (`PREREG_MANIFEST.txt`, Section E) (one dated immutable snapshot, coder C; provider-stable undated identifiers under the precommitted version-metadata-plus-B1c fallback, coders A and B). Disagreements were resolved by
...
| v0.13 | 2026-07-22 | Identifier completion: the preprint deposited to Zenodo; the archive-DOI slot filled with the concept DOI (10.5281/zenodo.21481634; initial version 10.5281/zenodo.21481635). No other change. |
...
### Draft revision note (v0.13, 2026-07-22)

Identifier completion only. The preprint was deposited to Zenodo (initial version DOI
10.5281/zenodo.21481635) after MetaArXiv declined the submission as outside its series'
scope; the availability section's "pending deposit" slot is filled with the concept DOI
(10.5281/zenodo.21481634), which resolves to the latest version. The companion essay's
DOI placeholder and the repository README are synced. Nothing else in the document was
altered.
...
This revision responds to a seventh external review (received 2026-07-21, of the v0.10
package), which approved the content and conditioned upload on finishing the vocabulary:
v0.10's sweep covered the "preregist-" family only, and the wider "register-" family — used
throughout for the project's own acts — survived it, contradicting the v0.9 correction at
scale. Inventory: **(1) The register-family sweep.** Sixty authorial-voice sites are
converted: "registered odds ratio" and its kin (predictions, thresholds, decisions, effect
sizes, fallbacks, rationales, power computations) become *precommitted*; "frozen at
registration" becomes *frozen at the protocol freeze*; "registration manifest" becomes the
*sealed manifest*; "the registration's operationalization/shorthand/labels" become the
*sealed plan's*; "self-registered study" becomes *self-sealed study*; verb uses ("the plan
registers," "hash is registered") become *records/recorded*. Four kinds of occurrence are
deliberately untouched, and the terminology note now says so: references to the OSF event
itself (the §4.4.9 correction, the deposit's own registration date), ledger nouns (the
decision register, the threshold register, the transparent-changes register, the gate-chain
registry), the linguistic sense of "register" (register leakage; hedged-register items), and
the proper noun Registered Reports — plus, as before, the frozen shells and the dated notes
below. **(2) Placeholder hygiene.** The availability section's bracketed archive-DOI
editorial placeholder is replaced by plain prose: "preprint archive DOI: pending deposit
(inserted in the first post-deposit version)." **(3) Scope of v0.10's "all repaired."** For
the record: that phrase quantified over the fourteen findings its own audit confirmed, not
over the vocabulary at large — the register-family residue this revision clears was outside
that audit's lens, as the reviewer correctly observed. This note, the version-history row, the
reading-guide range, and the terminology note's extension and relabeling are the bookkeeping
