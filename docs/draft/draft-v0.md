...
2. **Non-derivability, witness-certified.** S_i is not derivable from S_1, …, S_{i−1}; and this independence is certified by a **witness** — an extant scholarly position that accepts all prior steps and rejects, or in the boundary-marking limit formally suspends judgment on, S_i. (A reasoned refusal to affirm, issued from a position that has paid every prior toll, marks the boundary as effectively as denial; Table 1 flags its one suspension witness as such.)
...
**Table 1. The canonical eight-step partition, with design-phase priors to be tested (see text). Anchor citations are identifying exemplars fixed at design time, not comprehensive step bibliographies.**

| # | Thesis (the statement disputed) | Witness (accepts 1..i−1, rejects or suspends i; see note) | Anchor literature (design-phase set) | Qualitative prior (testable) | Type |
|---|---|---|---|---|---|
...
| S3 | The agent is unique, a se, uncreated; all else ontologically depends on it (*pantokrator*) | Bostrom's simulator (2003, structural — see note); Hartshorne 1984 (qualified — see note); Mormon finitism | gap-problem literature: Pruss 2009; Rasmussen 2009; Ocampo 2024 · bundling fence: Geach 1973; Hasker 1989 | contested / frozen — evidence up to S2 shared with contingent-creator hypotheses | α |
...
| S6 | The authentic bearer of that God's special revelation is the Hebrew prophetic tradition | Islam; Judaism (via the christological construal — see note); Hick's pluralism (1989) | contra: Alston's parity argument (1991) · pro anchor: Plantinga 2000 (a permissibility claim) | headwind / near-frozen; evidence defers to S7 | β·γ |
| S7 | Jesus of Nazareth was bodily raised (~AD 30) | Allison 2021 (suspension — see note); Lüdemann 1994 (step-contestation, not a chain-witness — see note) | pro: Wright 2003; Swinburne 2003 (with its own dwindling-probabilities caveat) | strong headwind; evidence-frozen (fixed corpus) | γ |
...
**Witness-verification note (v0.4).** A post-draft primary/secondary-source verification of the witness column (details in the audit package) confirmed the S1, S5, S6-Islam, and S8 assignments as stated — Russell denies both any cause of the totality and the legitimacy of the demand; Wiles holds S1–S4 fixed precisely to protect S4 from selective intervention; Islam accepts S1–S5 and rejects S6 on supersession grounds (Q 33:40; on the doctrine's universality in Sunni Islam, Friedmann 1986), with the *taḥrīf* charge as a differentiated second ground; Lapide accepts the resurrection "not as an invention of the community of disciples, but as a historical event" while denying every S8 inference from it — and qualified the assignments in four other cells, which the table now flags. *Bostrom* is a structural witness only: the trilemma's author accepts neither S1 nor S2 assertorically, so the simulation hypothesis demonstrates the logical independence of S3 from S1–S2 (an intentional external creator that is non-unique, dependent, possibly itself created) without occupying the stopping point; for assertoric theological uptake of the simulation hypothesis, see Steinhart (2010). *Hartshorne* affirms uniqueness, uncreatedness, and necessary existence; what he rejects is the classical aseity–omnipotence–creatio-ex-nihilo package, and his acceptance of S1 runs through the "ground" disjunct under panentheism. *Draper's indifference hypothesis and Law's evil-god* certify S4's boundary at the hypothesis level — seriously defended epistemic possibilities of S1–S3 without S4 — their authors being, respectively, a professed agnostic and an atheist; among the verified entries, no witness personally stops at S4 (Mill's finite-god position, the cell's third entry, was outside the verification pass and retains its design-phase status). *Judaism* affirms the bare revelatory authenticity of the Hebrew prophetic tradition (it supplies it); what it rejects, in a dedicated literature logically prior to any verdict on S7 (from Maimonides, *Mishneh Torah*, Hilkhot Melakhim 11:4, through the medieval polemical corpus edited in Berger 1979 and the Isaiah 53 exegetical tradition documented in Rembaum 1982), is the christological construal of that tradition which the chain's continuation requires — so S6's boundary is certified by two witnesses rejecting *different conjuncts* of the step's content, itself grain information anticipating the ten-step split in R1 (§6). *Lüdemann* rejects S7 but is not a chain-witness: the same 1994 work already discounts special divine action (S5), and he later renounced Christian belief; he is carried as step-contestation literature, with the S7 chain-boundary certified by *Allison's* suspension — which is second-order (about what unaided historical evidence warrants, while affirming the proposition as personal faith), and whose *non liquet* label is our characterization, not a quotation. None of these qualifications removes a boundary; each sharpens what kind of occupancy certifies it.
...
explained by run-level decoding noise (B1a/B1b), the preregistered B–C correlation watch did
...
registered in the plan. What remains is substantive disagreement among three model families about what fresh abstracts say, at a
...
accounting is invariant under notational bundling. (iv) *Canonicity, not uniqueness.* No
theorem here shows the witness criterion yields a unique maximal partition; "canonical" claims
admissibility and reproducibility of this partition under the stated criterion, and the
grain-robustness check (R1) exists precisely because rival admissible partitions may exist.
(v) *Grain-relativity.* The integer "eight" is relative to the witness criterion and to the state of the record that certifies it; the grain objection (§8, O1) lives exactly here. The reply is twofold: the paper's empirical hypotheses are formulated as invariants across admissible grains and are subjected to a preregistered grain-robustness check (R1, including the Mill finite-god cut); and the canonicity argument above ties admissible grains to occupied positions, so that the residual freedom in "eight" is the record's, not the authors'. What the formalism licenses, in the end, is modest and precise: a cumulative case may be stated in any vocabulary its proponent likes, but its burden ledger is representation-invariant, and the ledger's line items are fixed by the dialectical facts on the ground — facts the remainder of this paper undertakes to measure rather than presume.
...
scholarly position accepting all prior steps and rejecting (or, in the recognized limit, formally suspending judgment on) that one — with a dedicated
...
balance), and mainstream specialist editors and referees are drawn from that pool, so a
hostile-gatekeeping explanation of a pro-side prevalence pattern would require the gatekeepers
to be hostile to their own majority position. This is contextual evidence about composition,
not an identification of editorial or submission mechanisms; read that way, it makes the
design conservative on this axis — a point about the test's direction of stringency, not
about its outcome.
...
its validation battery, and the preregistered checkpoint that stopped the measurement. Whether
...
scheduled after the checkpoint) and remains sealed and hash-verifiable in the freeze manifest. The freeze manifest (`PREREG_MANIFEST.txt`, 64 artifacts plus aggregate hashes) is the verification root: any copy of a frozen artifact can be checked against its SHA256 independent of repository history. Concrete public identifiers — the OSF registration DOI, this preprint's archive DOI, and the repository URL — are inserted at upload; bracketed placeholders mark them in this draft. Copyrighted abstracts are not republished: the raw and sanitized
...
harness archived, so that coder D's column supports re-execution — not merely re-inspection — of
