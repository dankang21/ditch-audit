...
1. **가설 방화벽.** 코더에게 전달되는 프롬프트는 `scripts/build_coder_prompt.py`의
   ...
   `docs/p0-sweep.md`, `docs/validation-battery-v1.md`, `docs/synthetic-specs-v*.json`, `docs/analysis-plan-v*.md`,
...
4. **시크릿.** API 키는 `.env`에만. 커밋 금지. 코더는 서로 다른 세 모델 패밀리
   ...
   [잠정 판정자 — 확정 룰은 battery §B6]; coder D = 오픈웨이트 아카이브 **필수**.
   생성/세탁 E = GLM급 오픈웨이트 (Perplexity Agent API, `web_search` 미포함 =
   구조적 무검색; Sonar는 블라인드 역할 금지). 역할 배타 매트릭스는
...
|---|---|---|
| P1 ✅ | 파일럿 코딩: RS 2015 (실측 35) + 골드 20 | **G1: 전 차원, 3쌍 최소 pairwise Krippendorff α ≥ 0.70** |
| P2 ✅ | 코퍼스 수집 (T1 필터 + T2 2004–2024) | G2 PASS (층화 92.8%, 2026-07-17) |
| P3 ✅ | OSF 사전등록 freeze | **FREEZE 성립 2026-07-17 (guardian Run 2 CLEAN)** — OSF 웹 등록은 dk 잔여 액션 |
| P4 (다음) | 본 코딩 (~1,500 × 3코더) + 검증 배터리 전체 (인간 검증 없음 — 절대 규칙 5) | 체크포인트·비용 로그 필수 |
...
- [ ] 패밀리 E = GLM급 (Perplexity Agent API 경유, `web_search` 미포함) — `PERPLEXITY_API_KEY` 기입, 응답의 tool_calls_details 로깅 (battery §B0)
